    ...
    interface Props {
        order: Order,
        navigation: StackNavigationProp<ConductorOrderStackParamList, 'ConductorOrderListScreen', undefined>
    }
    ...
    export const OrderListItem = ({ order, navigation }: Props) => {
        ...
        return (
            <TouchableOpacity
            onPress={()=> navigation.navigate('ConductorOrderDetailScreen', {order:order})}
            >

                <View style={styles.container}>
                    <Text style={styles.order}>Orden #{order.id}</Text>
                    <Text style={{ ...styles.info, marginTop: 10 }}>Fecha el pedido: {DateFormatter(order.timestamp!)}</Text>
